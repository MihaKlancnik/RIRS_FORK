...
<Dialog.Root bind:open={isDialogOpen}>
    <Dialog.Trigger>
        <Button class="mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" data-testid="add-plane-dialog-button">
            Dodaj novo letalo
        </Button>
    </Dialog.Trigger>
    <Dialog.Content class="sm:max-w-[425px]">
        ...
        <div class="grid gap-4 py-4">
            ...
            <div class="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="registrska" bind:value={registrska_st} class="col-span-3" />
            </div>
            <div class="grid grid-cols-4 items-center gap-4">
                <Label for="id_flight" class="text-right">ID poleta</Label>
                <Input id="id_flight" type="number" bind:value={Polet_idPolet} class="col-span-3" />
            </div>
        </div>
        <Dialog.Footer>
